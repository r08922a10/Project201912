...
class LinearModel(nn.Module):
    ...
    def forward(self, input):
        ...
        #y = self.w(input)
        y = input[:,0].view(-1, 1) + 0.5*input[:,1].view(-1, 1)
...
def main():
    ...
    model.load_state_dict(torch.load("model2_nn2_69.pkl"))
    ...
    for epoch in range(100000):
        for i, xy in enumerate(train_loader):
            ...
            if epoch%1000 == 0:
                ...
                exit()
                '''
                val_label_freq, val_label_error = error(valid_predict, valid_y)
                label_freq, label_error = error(predict, y)
                
                label_freq = sorted(label_freq.items(), key=lambda d: d[0])
                label_error = sorted(label_error.items(), key=lambda d: d[0])
                val_label_freq = sorted(val_label_freq.items(), key=lambda d: d[0])
                val_label_error = sorted(val_label_error.items(), key=lambda d: d[0])
                #sorted(dict1.items(), key=lambda d: d[1])
                for i, k in enumerate(label_freq):
                    freq = label_freq[i][1]
                    #error_ = label_error[i][1]
                    print(label_freq[i][0], freq)
                print("Valid")
                for i, k in enumerate(val_label_freq):
                    freq = val_label_freq[i][1]
                    error_ = val_label_error[i][1]
                    print(label_freq[i][0], 1 - error_/freq)
                exit()
                '''
            if valid_acc > max_acc:
                max_acc = valid_acc
                print("Save Model ACC:", valid_acc)
                torch.save(model.state_dict(), "model/TP_nn_3.pkl")
